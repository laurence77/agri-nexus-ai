...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  const sanitizeCSSValue = (value: string): string => {
    // Remove potentially dangerous characters and limit to safe CSS color values
    return value.replace(/[^a-zA-Z0-9#().,\s%-]/g, '').substring(0, 50);
  };

  const sanitizeCSSIdentifier = (value: string): string => {
    // Only allow alphanumeric and hyphens for CSS identifiers
    return value.replace(/[^a-zA-Z0-9-]/g, '').substring(0, 30);
  };

  // Generate secure CSS with sanitized values
  const secureCSS = Object.entries(THEMES)
    .map(([theme, prefix]) => {
      const sanitizedPrefix = prefix; // Already controlled by THEMES constant
